...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  if (!isset($value['id']))
	...
	echo "<input type='hidden' name='{$name}[url]' value='{$value['url']}'/>";
	if ($has_image)
		echo "<input class='{$multiple} button  simplePanelimageUploadclear' id='{$id}' value='".__('Remove Image', 'g5plus-wolverine')."' type='button'/>";
	else
		echo "<input class='{$multiple} button simplePanelimageUpload' id='{$id}' value='".__('Upload Image', 'g5plus-wolverine')."' type='button'/>";
	$this->show_field_end( $field, $meta );
  }
