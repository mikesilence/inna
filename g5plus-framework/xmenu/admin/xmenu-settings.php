...
if ( !class_exists('XMenu_Setting_Options' ) ):
	class XMenu_Setting_Options {
		...
		function __construct() {
			...
			$this->setting_options = array(
				'settings' => array(
					...
					'config' => array(
						'setting-heading' => array(
							'text' => __('Settings','g5plus-wolverine'),
							'type' => 'heading',
						),

						/*'setting-responsive-breakpoint' => array(
							'text' => __('Responsive Breakpoint','g5plus-wolverine'),
							'type' => 'text',
							'std' => '',
						...
						'transition' => array(
							'text' => __('Select Menu Transition','g5plus-wolverine'),
							'type' => 'select',
							'std' => 'none',
							'options' => xmenu_get_transition()
						),
						'transition-duration' => array(
							'text' => __('Transition Duration','g5plus-wolverine'),
							'type' => 'text',
							'std' => '',
				...
				'images' => array(
					...
					'config' => array(
						'image-heading' => array(
							'text' => __('Images Settings','g5plus-wolverine'),
							'type' => 'heading',
						),
						'image-size' => array(
		...
		function admin_menu() {
			add_theme_page(
				__('XMENU Settings','g5plus-wolverine'),
				__('XMENU Settings','g5plus-wolverine'),
				'manage_options',
				'xmenu-settings',
		...
		function plugin_page() {
			...
			<div class="xmenu-settings">
				<h2>
					<a class="tab <?php echo ($current_menu == '' ? 'active' : '') ?>" href="themes.php?page=xmenu-settings"><strong><?php _e('XMENU','g5plus-wolverine') ?></strong> <?php _e('Setting','g5plus-wolverine') ?></a>
					<?php foreach ($setting_menus as $setting_menu): ?>
						<a class="tab <?php echo ($current_menu == $setting_menu ? 'active' : '') ?>" href="<?php echo esc_url('themes.php?page=xmenu-settings&menu=' . $setting_menu) ?>"><strong><?php echo ($setting_menu); ?></strong> <?php _e('Setting','g5plus-wolverine') ?></a>
					<?php endforeach; ?>
					<a class="tab" href="themes.php?page=xmenu-settings&do=widget"><strong><?php _e('XMENU','g5plus-wolverine') ?></strong> <?php _e('Widget Manager','g5plus-wolverine') ?></a>
					<button id="setting-add-menu" type="button" class="button button-primary x-fr"><?php _e('Add Menu Setting','g5plus-wolverine') ?></button>
				</h2>
				<form method="post" action="themes.php?page=xmenu-settings" novalidate="novalidate" id="xmenu_settings">
					...
					<p class="submit">
						...
						<button type="button" id="xmenu-save-setting" class="button button-primary"><i class="fa fa-save"></i> <?php echo __('Save Changes','g5plus-wolverine')?></button>
						<?php if (!empty($current_menu)): ?>
							<input id="xmenu_menu_slug" type="hidden" name="menu_slug" value="<?php echo esc_attr($current_menu) ?>"/>
							<button type="button" class="button button-primary" id="xmenu-delete-setting"><i class="fa fa-remove"></i> <?php echo __('Delete Settings','g5plus-wolverine')?></button>
						<?php endif;?>
					</p>
				</form>
				<div class="x-popup" id="xmenu-setting-popup">
					<div class="x-popup-inner">
						<h3><?php _e('Select Menu for your new settings','g5plus-wolverine') ?> <i id="setting-add-menu-close" class="fa fa-close"></i></h3>
						<div class="x-popup-select">
							...
							<button id="xmenu-create-button" type="button"><?php _e('Create Menu Setting','g5plus-wolverine') ?></button>
						</div>
					</div>
				</div>
			</div>
		...
		function widget_manager() {
			...
			<div class="xmenu-settings">
				<h2>
					<a class="tab" href="themes.php?page=xmenu-settings"><strong><?php _e('XMENU','g5plus-wolverine') ?></strong> <?php _e('Setting','g5plus-wolverine') ?></a>
					<?php foreach ($setting_menus as $setting_menu): ?>
						<a class="tab" href="<?php echo esc_url('themes.php?page=xmenu-settings&menu=' . $setting_menu)  ?>"><strong><?php echo esc_html($setting_menu); ?></strong> <?php _e('Setting','g5plus-wolverine') ?></a>
					<?php endforeach; ?>
					<a class="tab active" href="themes.php?page=xmenu-settings&do=widget"><strong><?php _e('XMENU','g5plus-wolverine') ?></strong> <?php _e('Widget Manager','g5plus-wolverine') ?></a>
				</h2>
						...
						<tbody>
							<?php foreach( $widgets as $menu_item_id => $widget_title ):?>
								<tr>
									...
									<td><?php echo esc_html($widget_title) . __(' [Menu Item ', 'g5plus-wolverine') . esc_html($menu_item_id) .']'; ?></td>
								</tr>
							<?php endforeach;?>
						</tbody>
					</table>
					<p><?php _e('Choose which Custom Widget Areas to delete', 'g5plus-wolverine') ?></p>
					<p class="submit"><input type="submit" name="submit" id="submit" class="button button-primary" value="<?php echo __('Delete Widget Areas','g5plus-wolverine')?>"></p>
				</form>
			</div>
			<?php
		}
		...
		function xmenu_setting_create() {
			$error_result = array(
				'code' => '-1',
				'message' => __('Create menu settings fail','g5plus-wolverine'),
			);
		...
		function xmenu_delete_setting() {
			$error_result = array(
				'code' => '-1',
				'message' => __('Delete menu settings fail','g5plus-wolverine'),
			);
