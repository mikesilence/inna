	...
	class G5Plus_Install_Demo
	{
		...
		function  install_demo() {
			if (!(isset($_REQUEST['security']) && current_user_can( 'manage_options' )) )
			{
				...
				$data_response = array(
					'code' => 'error',
					'message' => __("Permission error!",'g5plus-wolverine')
				);
			...
			if ($importer_error !== false) {
				...
				$data_response = array(
					'code' => 'fileNotFound',
					'message' => __("The Auto importing script could not be loaded. please use the wordpress importer and import the XML file that is located in your themes folder manually.",'g5plus-wolverine')
				);
				echo json_encode($data_response);
				die();
			}
				...
				switch (trim($type)) {
					case 'init':
						 ...
						 * Remove log file
						 */
						if ( $handle = opendir( get_template_directory() . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'data-demo' . DIRECTORY_SEPARATOR . "log" ) ) {
							while ( false !== ( $entry = readdir( $handle ) ) ) {
								if ( $entry != "." && $entry != ".." ) {
									unlink( get_template_directory() . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'data-demo' . DIRECTORY_SEPARATOR . "log" . DIRECTORY_SEPARATOR . $entry );
								}
							}
						}
					...
					case 'fix-data':
						...
						foreach ( $arr_demo_file as $file_demo ) {
							if (!file_exists($file_demo)) {
								...
								$data_response = array(
									'code' => 'fileNotFound',
									'message' => __("File not found! Please check file exists in directory:\n[your-theme]/assets/data-demo/",'g5plus-wolverine')
								);
								echo json_encode($data_response);
								die();
							}
