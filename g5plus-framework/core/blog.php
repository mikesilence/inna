	...
	function g5plus_paging_load_more() {
		...
		if (!empty($link)) :
			...
			<div class="blog-load-more-wrapper">
				<button data-href="<?php echo esc_url($link); ?>" type="button"  data-loading-text="<span class='fa fa-spinner fa-spin'></span> <?php _e("Loading...",'g5plus-wolverine'); ?>" class="blog-load-more wolverine-button style1 button-2x" autocomplete="off">
					<span class="fa fa-plus"></span> <?php _e("Load More",'g5plus-wolverine'); ?>
				</button>
			</div>
		<?php
		endif;
                ...
                <div class="jp-gui jp-interface">
                    <ul class="jp-controls jp-play-pause">
                        <li><a href="javascript:;" class="jp-play" tabindex="1"><i
                                    class="fa fa-play"></i> <?php _e('play', 'g5plus-wolverine'); ?></a></li>
                        <li><a href="javascript:;" class="jp-pause" tabindex="1"><i
                                    class="fa fa-pause"></i> <?php _e('pause', 'g5plus-wolverine'); ?></a></li>
                    </ul>
                    ...
                    <ul class="jp-controls jp-volume">
                        <li>
                            <a href="javascript:;" class="jp-mute" tabindex="1" title="mute"><i
                                    class="fa  fa-volume-up"></i> <?php _e('mute', 'g5plus-wolverine'); ?></a>
                        </li>
                        <li>
                            <a href="javascript:;" class="jp-unmute" tabindex="1" title="unmute"><i
                                    class="fa fa-volume-off"></i><?php _e('unmute', 'g5plus-wolverine'); ?></a>
                        </li>
                        <li>
                    ...
                    <ul class="jp-toggles">
                        <li>
                            <a href="javascript:;" class="jp-shuffle" tabindex="1"
                               title="shuffle"><?php _e('shuffle', 'g5plus-wolverine'); ?></a>
                        </li>
                        <li>
                            <a href="javascript:;" class="jp-shuffle-off" tabindex="1"
                               title="shuffle off"><?php _e('shuffle off', 'g5plus-wolverine'); ?></a>
                        </li>
                        <li>
                            <a href="javascript:;" class="jp-repeat" tabindex="1"
                               title="repeat"><?php _e('repeat', 'g5plus-wolverine'); ?></a>
                        </li>
                        <li>
                            <a href="javascript:;" class="jp-repeat-off" tabindex="1"
                               title="repeat off"><?php _e('repeat off', 'g5plus-wolverine'); ?></a>
                        </li>
                    </ul>
                </div>

                <div class="jp-no-solution">
                    <?php printf(__('<span>Update Required</span> To play the media you will need to either update your browser to a recent version or update your <a href="%s" target="_blank">Flash plugin</a>.', 'g5plus-wolverine'), 'http://get.adobe.com/flashplayer/'); ?>
                </div>
            </div>
        </div>
        <?php
    ...
    function g5plus_post_meta_small() {
        ...
        <ul class="entry-meta-small">
            <li class="entry-meta-author">
                <?php _e('By','g5plus-wolverine') ?> : <?php printf('<a href="%1$s">%2$s</a>', esc_url(get_author_posts_url(get_the_author_meta('ID'))), esc_html(get_the_author())); ?>
            </li>
            <li class="entry-meta-read-more">
                <a href="<?php the_permalink(); ?>" rel="bookmark" title="<?php the_title(); ?>"><?php _e('Read more','g5plus-wolverine') ?></a>
            </li>
        </ul>
        <?php
        $content = ob_get_clean();
...
if (!function_exists('g5plus_link_pages')) {
    function g5plus_link_pages() {
        wp_link_pages(array(
            'before' => '<div class="g5plus-page-links"><span class="g5plus-page-links-title">' . __('Pages:', 'g5plus-wolverine') . '</span>',
            'after' => '</div>',
        ...
        <li <?php comment_class(); ?> id="li-comment-<?php comment_ID() ?>">
            <div id="comment-<?php comment_ID(); ?>" class="comment-body clearfix">
                ...
                <div class="comment-text">
                    <div class="author">
                        <div class="author-name"><?php printf(__('%s', 'g5plus-wolverine'), get_comment_author_link()) ?></div>
                        <div class="comment-meta">
                            <span class="comment-meta-date">
                                <?php echo get_comment_date('M j, Y'); ?>
                            </span>

                            <?php edit_comment_link(__('Edit', 'g5plus-wolverine'), '<span class="comment-meta-edit">', '</span>') ?>
                        </div>
                    </div>
                    <div class="text"><?php comment_text() ?></div>
                    <?php if ($comment->comment_approved == '0') : ?>
                        <em><?php _e('Your comment is awaiting moderation.', 'g5plus-wolverine') ?></em>
                    <?php endif; ?>
                    <div class="comment-meta-bottom">
                        <span class="comment-like">
                            <a data-like-comment="true" data-id="<?php comment_ID(); ?>" href="javascript:;">
                                <?php _e('Like:','g5plus-wolverine') ?> <label><?php echo get_comment_meta(get_comment_ID(),'g5plus-like',true) == '' ? 0 : get_comment_meta(get_comment_ID(),'g5plus-like',true); ?></label> </a>
                        </span>
    ...
    function g5plus_comment_form() {
        ...
        $fields = array(
            'author' => '<div class="form-group col-md-12">' .
                '<label for="author">'.__('Name*','g5plus-wolverine').'</label>'.
            ...
            'email' => '<div class="form-group col-md-12">' .
                '<label for="email">'.__('Email*','g5plus-wolverine').'</label>'.
            ...
            'url'   => '<div class="form-group col-md-12">'.
                '<label for="url">'.__("Website",'g5plus-wolverine').'</label>'.
                '<input id="url" name="url" ' . ( $html5 ? 'type="url"' : 'type="text"' ) . ' value="' . esc_attr( $commenter['comment_author_url'] ) . '" placeholder="'.__('Website','g5plus-wolverine').'" />'.
                '</div>'
        );
        ...
        $comment_form_args = array(
            ...
            'comment_field' => '<div class="form-group col-md-12">' .
                '<label for="comment">'.__('Message*','g5plus-wolverine').'</label>'.
                '<textarea rows="6" id="comment" name="comment" placeholder="'.__('Message*','g5plus-wolverine') .'" '. $aria_req .'></textarea>' .
                '</div>',
            'comment_notes_before' => '<p class="comment-notes">' .
                __('Your email address will not be published.', 'g5plus-wolverine') /* . ( $req ? $required_text : '' ) */ .
                '</p>',
            'comment_notes_after' => '',
            'id_submit' => 'btnComment',
            'class_submit' => 'button-comment',
            'title_reply' => __('Leave a Comment', 'g5plus-wolverine'),
            'title_reply_to' => __('Leave a Comment to %s', 'g5plus-wolverine'),
            'cancel_reply_link' => __('Cancel reply', 'g5plus-wolverine'),
            'label_submit' => __('Send', 'g5plus-wolverine')
        );
